...
final as (
    ...
    select
        ...
        cast( {{ twilio.remove_non_numeric_chars('num_media')}} as {{ dbt.type_float() }}) as num_media,
        cast( {{ twilio.remove_non_numeric_chars('num_segments')}} as {{ dbt.type_float() }}) as num_segments,
